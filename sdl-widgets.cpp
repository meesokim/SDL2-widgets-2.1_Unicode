...
void TopWin::fullscreen(int x) {
	SDL_SetWindowFullscreen(window, x);
}
...
void Label::draw(Render rend,Point pnt) {
  if (wstr)	render_t->draw_string(rend,wstr,pnt);
  else if (str) render_t->draw_string(rend,str,pnt);
...
int RenderText::draw_string(Render rend,const wchar_t *s,Point pt) {
  Rect offset(pt.x,pt.y,0,0);
  wchar_t ind;
  for (int i=0;s[i] && offset.x < rend.surf->w;++i) {
    if (s[i]=='\n') {
    ...
    if (!chars[ind]) set_char(ind);
    Rect off(offset);
    SDL_BlitSurface(wchars[ind],0,rend.surf,&off); // off.w end off.h might be modified!
    offset.x+=ch_wwid[ind];
  }
  return offset.y + TDIST;
}

void RenderText::set_char(const wchar_t ind) {
  static unsigned short buf[2];
  buf[0]=(unsigned short) (ind+' ');
  buf[1]=0;
  wchars[ind]=TTF_RenderUNICODE_Blended(ttf_font,buf,text_col);
  ch_wwid[ind]=wchars[ind]->w;
}
...
int RenderText::text_width(const wchar_t *s) {
  int wid=0;
  if (!s) return char_wwid;
  for (int i=0;s[i];++i) {
    if (s[i]<' ') {
      alert("text_width: bad char in string %s",s);
      return 0;
