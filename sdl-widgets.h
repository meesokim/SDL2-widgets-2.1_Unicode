...
struct RenderText {
  ...
  static const int wdim = 65536 - ' ';
  SDL_Surface *chars[dim+1];
  int ch_wid[dim+1];
  SDL_Surface *wchars[wdim+1];
  int ch_wwid[wdim+1];
  TTF_Font *ttf_font;
  SDL_Color text_col;
  RenderText(TTF_Font*,SDL_Color text_col);
  int draw_string(Render rend,const char *s,Point);
  int draw_string(Render rend,const wchar_t *s,Point);
  int text_width(const char *s);
  int text_width(const wchar_t *s);
  void set_char(const int ind);
  void set_char(const wchar_t ind);
};
